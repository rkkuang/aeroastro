...
class Nlenses(object):
    ...
    def img_mapping_inone_v2(self,srcplaneIMG, imgplaneIMG, srcplaneIMG_withoutlens, inxdata, inydata, betainxdata, betainydata, xlim, ylim, ImgSize,valarr1 = None,valarr2 = None, datatype=np.float64):
    ...
    def genimgs_fast(self, ImgSize, xlim, ylim, num,datatype=np.float):
        # # not use mag in computing src plane map
        srcplaneIMG = np.zeros(ImgSize).astype(datatype)
        srcplaneIMG_withoutlens = np.zeros(ImgSize).astype(datatype)
        imgplaneIMG = np.zeros(ImgSize).astype(datatype)
        ...
        # betax, betay, MAG = self.ray_shoot_comp_mag_in_one(thetax, thetay)
        betax, betay = self.inverse_ray_shooting(thetax=thetax, thetay=thetay)
        srcplaneIMG, imgplaneIMG, srcplaneIMG_withoutlens = self.img_mapping_inone_v2(srcplaneIMG, imgplaneIMG, srcplaneIMG_withoutlens,thetax, thetay,betax, betay, xlim, ylim, ImgSize,valarr1 = np.ones([len(thetax),]), valarr2 = np.ones([len(thetax),]), datatype=datatype)
        # pbar.update(1)
        srcplaneIMG /= srcplaneIMG_withoutlens
        return srcplaneIMG, imgplaneIMG, srcplaneIMG_withoutlens
...
def genimgs(xylim = 6, q = 0.5, posscale = 0.5, imgsz = 2048, num = 2000, method = "slow"):
    ...
    if method == "fast":
        ...
        srcplaneIMG, imgplaneIMG, srcplaneIMG_withoutlens = twolens.img_mapping_inone(thetax, thetay,twolens.betax, twolens.betay, xlim, ylim, ImgSize,valarr1 = np.ones([len(thetax),]), valarr2 = np.ones([len(thetax),]), datatype=datatype)
        t1 = time.time()
    elif method == "slow":
        t0 = time.time()
